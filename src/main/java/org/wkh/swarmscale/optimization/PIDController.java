 ...
 * Minimal usage:<br>
 * PIDController pid = new PIDController(p,i,d); <br>
 * ...looping code...{ <br>
...
public class PIDController {
     ...
     * @param d Derivative gain. Responds quickly to large changes in error. Small values prevents P and I terms from
     * causing overshoot.
     * @param f Feed-forward gain. Open loop "best guess" for the output should be. Only useful if setpoint represents a
     * rate.
     */
     ...
     * </ul>
     * A small D value can be useful for both improving response times, and preventing overshoot. However, in many
     * systems a large D value will cause significant instability, particularly for large setpoint changes.
     * <br>
     ...
     * This is excellent for velocity, rate, and other continuous control modes where you can expect a rough output
     * value based solely on the setpoint.<br>
    ...
    /**
     * Set the maximum output value contributed by the I component of the system This can be used to prevent large
     * windup issues and make tuning simpler
     *
     ...
     * This should be used any time the PID is disabled or inactive for extended duration, and the controlled portion of
     * the system may have changed due to external forces.
     */
    public void reset() {
        firstRun = true;
        errorSum = 0;
     ...
     * This can prevent sharp jumps in output when changing setpoints or enabling a PID system, which might cause stress
     * on physical or electrical systems.  <br>
     * Can be very useful for fast-reacting control loops, such as ones with large P or D values and feed-forward
     * systems.
     *
     ...
     * <br>This limits the reactivity of P term, and restricts impact of large D term during large setpoint adjustments.
     ...
     * 0.1 is likely a sane starting value. Larger values use historical data more heavily, with low values weigh newer
     * data. 0 will disable, filtering, and use only the most recent value. <br>
     * Increasing the filter strength will P and D oscillations, but force larger I values and increase I term
     * overshoot.<br>
    ...
    /**
     * To operate correctly, all PID parameters require the same sign This should align with the {@literal}reversed
     * value
     */
    ...
    public static void main(String[] args) {
        PIDController controller = new PIDController(0.25, 0.01, 0.4);
        controller.setOutputLimits(10);
        //miniPID.setMaxIOutput(2);
        //miniPID.setOutputRampRate(3);
        //miniPID.setOutputFilter(.3);
        controller.setSetpointRange(40);
        ...
        for (int i = 0; i < 100; i++) {
            ...
            output = controller.getOutput(actual, target);
